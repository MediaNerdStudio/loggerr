...
export class RecordingEngine extends EventEmitter {
  ...
  start(recording) {
    ...
    const inputArgs = recording.sourceType === 'ingest'
      ...
      : ['-reconnect', '1', '-reconnect_streamed', '1', '-reconnect_delay_max', '5', '-i', recording.sourceUrl];
    ...
    const state = { state: 'starting', startedAt: new Date().toISOString(), currentFile: null, directory: path.dirname(absolutePattern), error: null, stderr: '', child: null };
    const child = spawn(process.env.FFMPEG_PATH || 'ffmpeg', args, { windowsHide: true });
    state.child = child;
    state.state = 'recording';
    this.processes.set(recording.id, state);
    child.stderr.on('data', chunk => { state.stderr = `${state.stderr}${chunk}`.slice(-4000); state.currentFile = this.latestFile(path.dirname(absolutePattern)); this.emit('status'); });
    child.on('error', error => { state.state = 'error'; state.error = error.message; this.emit('status'); });
    child.on('exit', (code, signal) => { this.processes.delete(recording.id); if (code && signal !== 'SIGTERM') this.emit('failure', recording.id, state.stderr || `FFmpeg exited with code ${code}`); this.emit('status'); });
    this.emit('status');
  }
